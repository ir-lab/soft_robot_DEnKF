...
class transform:
    def __init__(self):
        ...
        parameters = pickle.load(open('./dataset/processed_data/parameter_52.pkl', 'rb'))
...
class inference:
    ...
    def test(self):
        ...
        if torch.cuda.is_available():
            ...
            checkpoint = torch.load('continue-model-178800', map_location=torch.device('cpu'))
            self.model.load_state_dict(checkpoint['model'])
        self.model.eval()
        ...
        '''
        data = pickle.load(open('./dataset/processed_data/test_dataset_52.pkl', 'rb'))
        ...
        for i in range (2000):
            if i == 0:
                ...
                state_ensemble = rearrange(state_ensemble, '(bs k) dim -> bs k dim', bs=1)
