...
class inference:
    def __init__(self):
        ...
        self.forward_model_ = Forward_model(self.num_ensemble, self.dim_x, self.dim_a)
        # Check model type
        ...
        if torch.cuda.is_available():
            checkpoint = torch.load("continue-model-238200")
            self.model.load_state_dict(checkpoint["model"])
        else:
            checkpoint = torch.load(
                "continue-model-238200", map_location=torch.device("cpu")
            )
            self.model.load_state_dict(checkpoint["model"])
        self.model.eval()
        ...
        # print(self.forward_model_)

        # for name, child in self.model.named_children():
        #     for name2, params in child.named_parameters():
        #         print(name, name2)
        # print(self.model.process_model.state_dict())
    ...
    def generate_mask(self, selection):
        ...
        i.e. selection = [1] -> remove the 1st imu readings
             selection = [3,4] -> remove the 3td, and 4th imu readings
        """
        if len(selection) == 0:
            index = []
        else:
    ...
    def test(self):
        ...
        make sure the data comes in the same format as the .pkl file
        once the data format is correct, the model is runnable
        """
        ...
        for i in range(data["obs"].shape[0]):
            ...
            NEED MODIFY!!!
            This is the entry point for ROS, I am now just using the recorded data 
            for real-time deployment, pass ROS topic data and process the data
            ...
            code = data["code"][i]
            sample_freq = torch.tensor(data["sample_freq"][i], dtype=torch.float32)
            ...
            sample_freq = repeat(sample_freq, "(bs k) dim -> bs k dim", bs=1)
            ...
            with torch.no_grad():
                if i == 0:
                    ...
                    state = state
                    input_state = (ensemble, state)
                    # let's use the forward model after 1000 point
                    if i >= 2000:
                        output = self.forward_model_(input_state, action, sample_freq)
                        ensemble = output[0]  # -> ensemble estimation
                        state = output[1]  # -> final estimation
                    else:
                        obs_action = (action, obs, sample_freq)
                ...
                # final_obs = obs_p
                # final_obs = rearrange(final_obs, "bs k dim -> (bs k) dim", k=1)
                # final_obs = self.transform_.state_inv_transform(final_obs)
                # final_obs = rearrange(final_obs, "(bs k) dim -> bs k dim", bs=1)

                final_state = final_state.cpu().detach().numpy()
                state_gt = state_gt.cpu().detach().numpy()
                # final_obs = final_obs.cpu().detach().numpy()
